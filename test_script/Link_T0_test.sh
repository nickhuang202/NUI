...
if [ "$ZST_VER" == "" ] || [ "$TOPOLOGY_NAME" == "" ];then
	...
	echo "Usage: ./Link_T0_test.sh <zst_version> <topology_name> [test_cases]"
	echo "Example: ./Link_T0_test.sh $BIN default"
	echo ""
...
LINK_TEST_LOG=t0_link_test_"$ZST_VER"_"$DATE".log
...
case "$DETECTED_PLATFORM" in
	MINIPACK3BA)
    ./bin/run_test.py link --agent-run-mode mono --filter_file ./share/hw_sanity_tests/t0_ensemble_link_tests.conf --config $AGENT_CONFIG --qsfp-config $QSFP_CONFIG --known-bad-tests-file ./share/link_known_bad_tests/agent_ensemble_link_known_bad_tests.materialized_JSON --skip-known-bad-tests "montblanc/sai/asicsdk-13.3.0.0_odp/13.3.0.0_odp" --mgmt-if eth0 2>&1 | tee "$LINK_TEST_LOG"
		;;
	MINIPACK3N)
    ./bin/run_test.py link --agent-run-mode mono --filter_file ./share/hw_sanity_tests/t0_ensemble_link_tests.conf --config $AGENT_CONFIG --qsfp-config $QSFP_CONFIG --known-bad-tests-file ./share/link_known_bad_tests/agent_ensemble_link_known_bad_tests.materialized_JSON --skip-known-bad-tests "montblanc/sai/asicsdk-13.3.0.0_odp/13.3.0.0_odp" --mgmt-if eth0 2>&1 | tee "$LINK_TEST_LOG"
		;;
	WEDGE800BACT)
    ./bin/run_test.py link --agent-run-mode mono --filter_file ./share/hw_sanity_tests/t0_ensemble_link_tests.conf --config $AGENT_CONFIG --qsfp-config $QSFP_CONFIG --known-bad-tests-file ./share/link_known_bad_tests/agent_ensemble_link_known_bad_tests.materialized_JSON --skip-known-bad-tests "montblanc/sai/asicsdk-13.3.0.0_odp/13.3.0.0_odp" --fruid-path=$FRU_CONFIG --mgmt-if eth0  2>&1 | tee "$LINK_TEST_LOG"
		;;
	WEDGE800CACT)
...
FINAL_ARCHIVE="LINK_T0_${DETECTED_PLATFORM}_${ZST_VER}_${TOPOLOGY_NAME}_${DATE}.tar.gz"
